...
#doc (Page) "Real analysis and linear arithmetic" =>

This lecture teaches you some logic in Lean!
...
In particular, Lean can not calculate with the real numbers. But we can use the `norm_num`
tactic to show simple identities of numbers.
```lean demo
example : (2 : ℝ) + 2 = 4 := by
  norm_num
```
...
lemma ConvergesTo.add {a b : ℕ → ℝ} {x y : ℝ}
  ...
  use n₀ + m₀
  intro n hn
  have h1 : |x - a n| ≤ ε / 2 := sorry
  have h2 : |y - b n| ≤ ε / 2 := sorry
  calc
    |x + y - (a + b) n| = |x + y - (a n + b n)| := by sorry
                      _ = |(x - a n) + (y - b n)| := by sorry
                      _ ≤ |x - a n| + |y - b n| := by sorry
                      _ ≤ ε / 2 + ε / 2 := by sorry
                      _ = ε := by simp
```

Let us define _bounded sequences_: A sequence $`(a_n)_{n \in \mathbb{N}}` is _bounded_ if
there exists a constant $`M` such that $`|a_n| \le M` for all $`n`.
...
As the final challenge, let's prove:
If $`a` converges to $`x` and $`b` converges to $`y`, then $`ab` converges to $`xy`.
